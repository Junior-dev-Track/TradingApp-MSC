...
export default function Dashboard({ auth }: PageProps = {}) {
    ...
    const addPurchase = (stock: BarData, quantity: number) => {
        ...
        if (availableFunds >= totalPrice) {
            ...
            addNotification(
                `${quantity} shares of ${stock.symbol} have been purchased.`
            );
            updateNetGainLoss();
        } else {
            addNotification(
    ...
    const sellAsset = (symbol: string) => {
        ...
        if (assetToSell) {
            setAvailableFunds(
                (prevFunds) => prevFunds + (assetToSell.totalPrice || 0)
            );
            setTotalBalance(
                (prevBalance) => prevBalance + (assetToSell.totalPrice || 0)
            );
    ...
    const updateNetGainLoss = () => {
        ...
        const currentNetGainLoss =
            availableFunds + totalInvested - initialFunds;
            ...
            <div className="flex">
                ...
                <div className="w-3/7 py-1 p-1 w-10/12 mr-16">
                    <div className="grid grid-cols-3 gap-4">
                        <div
                            className={`col-span-3 bg-gray-700 p-3 h-15 rounded-lg shadow ${
                        ...
                        <div
                            className={`bg-gray-700 p-3 rounded-lg shadow h-30 overflow-y-hidden col-span-1 ${
                        ...
                        >
                            ...
                            >
                                {netGainLoss >= 0 ? "Profit" : "Loss"}: $
                                {Math.abs(netGainLoss).toFixed(2)}
                            </div>
                        </div>

                        <div
                            className={`bg-gray-700 p-3 rounded-lg shadow h-30 overflow-y-auto col-span-2 ${
                        ...
                        <div
                            className={`bg-gray-700 p-3 rounded-lg shadow h-30 overflow-y-auto col-span-3 ${
